...
**No API key required to explore.** When both `GEMINI_API_KEY` and `OPENAI_API_KEY` are missing, the app automatically switches to Demo Mode — all AI responses (questions, evaluations, scores, feedback) are replaced with realistic simulated data so you can explore every feature without any setup.
...
# AI provider: auto | gemini | openai
AI_PROVIDER=auto

# Recommended: Gemini free-tier key (backend only, never exposed to frontend)
...
- both `GEMINI_API_KEY` and `OPENAI_API_KEY` are missing or empty
